-- =====================================================
-- FINAL PRICE CHANGE NOTIFICATIONS SYSTEM
-- =====================================================
-- This is the working version - run this to set everything up cleanly
...
DROP TRIGGER IF EXISTS horse_price_change_trigger ON horses CASCADE;
DROP FUNCTION IF EXISTS notify_watchers_on_price_drop() CASCADE;
DROP FUNCTION IF EXISTS track_price_change() CASCADE;
DROP FUNCTION IF EXISTS track_horse_price_change() CASCADE;
...
-- =====================================================
-- STEP 3: CREATE PRICE TRACKING FUNCTION
-- =====================================================
CREATE OR REPLACE FUNCTION track_price_change()
RETURNS TRIGGER AS $$
BEGIN
  -- Debug logging
  RAISE NOTICE 'Price change trigger fired. Horse ID: %, OLD.price: %, NEW.price: %', NEW.id, OLD.price, NEW.price;

  -- Only track if:
  -- 1. This is an UPDATE (OLD exists)
  -- 2. Both old and new prices are NOT NULL
  -- 3. Price actually changed
  IF OLD IS NOT NULL
    ...
    RAISE NOTICE 'Price history inserted successfully';
  ELSE
    RAISE NOTICE 'Skipping price history. OLD exists: %, OLD.price: %, NEW.price: %, Prices different: %',
      (OLD IS NOT NULL),
      OLD.price,
      NEW.price,
...
-- =====================================================
-- STEP 4: CREATE TRIGGER (WITHOUT WHEN CLAUSE)
-- =====================================================
-- Removing WHEN clause to ensure trigger always fires
...
BEGIN
  ...
  IF NEW.price_change < 0 THEN
    ...
    RAISE NOTICE 'Price drop detected: $% -> $%', NEW.old_price, NEW.new_price;
    ...
    IF horse_record IS NULL THEN
      RAISE NOTICE 'Horse not found for ID: %', NEW.horse_id;
      RETURN NEW;
    END IF;

    RAISE NOTICE 'Found horse: %', horse_record.name;
    ...
    LOOP
      RAISE NOTICE 'Creating notification for user: %', watcher.user_id;
    ...
    END LOOP;
  ELSE
    RAISE NOTICE 'Price increased, not sending notifications';
  END IF;
...
-- =====================================================
-- STEP 6: CREATE NOTIFICATION TRIGGER
-- =====================================================
...
WHERE event_object_table IN ('horses', 'price_history')
