-- =====================================================
-- CLEAN INSTALL OF PRICE CHANGE NOTIFICATIONS
-- =====================================================
-- Run this to cleanly set up price notifications
...
DROP FUNCTION IF EXISTS notify_watchers_on_price_drop();
DROP FUNCTION IF EXISTS track_price_change();
DROP FUNCTION IF EXISTS get_horse_price_history(UUID, INT);
DROP FUNCTION IF EXISTS get_horse_lowest_price(UUID);
DROP FUNCTION IF EXISTS get_horse_highest_price(UUID);
DROP FUNCTION IF EXISTS has_recent_price_drop(UUID, INT);
...
BEGIN
  ...
  -- (Skip tracking for new horses or if old price was null)
  IF OLD IS NOT NULL
...
-- Attach trigger to horses table (only on UPDATE, not INSERT)
DROP TRIGGER IF EXISTS track_horse_price_changes ON horses;
